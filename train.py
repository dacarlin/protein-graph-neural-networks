...
# Global alphabet 
aa_to_idx = {'ALA': 0, 'ARG': 1, 'ASN': 2, 'ASP': 3, 'CYS': 4, 'GLN': 5, 'GLU': 6, 'GLY': 7, 'HIS': 8, 'ILE': 9, 'LEU': 10, 'LYS': 11, 'MET': 12, 'PHE': 13, 'PRO': 14, 'SER': 15, 'THR': 16, 'TRP': 17, 'TYR': 18, 'VAL': 19}
...
class ProteinLigandGNN(nn.Module):
    ...
    def __init__(self, protein_features, ligand_features, edge_features, hidden_dim, num_layers):
    ...
    def forward(self, x, edge_index, edge_attr, batch, ligand_x, ligand_batch):
        ...
        if ligand_x.size(0) > 0:
            ...
            ligand_pooled = torch.zeros((1, self.protein_embedding.out_features), device=protein_x.device)
...
def phi_psi_atoms(residues):
    ...
    if 'C' in residues[0] and 'N' in residues[1] and 'CA' in residues[1] and 'C' in residues[1]:
        phi_atoms = (residues[0]['C'], residues[1]['N'], residues[1]['CA'], residues[1]['C'])

    if 'N' in residues[1] and 'CA' in residues[1] and 'C' in residues[1] and 'N' in residues[2]:
        psi_atoms = (residues[1]['N'], residues[1]['CA'], residues[1]['C'], residues[2]['N'])
...
def get_backbone_dihedrals(residues):
    ...
    for i in range(len(residues)):
        ...
        if i > 0:
            ...
            omega_angle = calculate_dihedral(torch.tensor(prev_c), torch.tensor(curr_n), torch.tensor(curr_ca), torch.tensor(curr_c))
        else:
            omega_angle = 0.0
        ...
        if 'CB' in residues[i]:
            ...
            cg = next((residues[i][atom].coord for atom in ['CG', 'SG', 'OG', 'OG1'] if atom in residues[i]), None)
            if cg is not None:
                chi1_angle = calculate_dihedral(torch.tensor(n), torch.tensor(ca), torch.tensor(cb), torch.tensor(cg))
            else:
                chi1_angle = 0.0
        else:
            chi1_angle = 0.0
...
def load_protein_ligand_graph(pdb_file):
    ...
    print(aa_to_idx)
    y = F.one_hot(torch.tensor([aa_to_idx[aa] for aa in amino_acids]), num_classes=NUM_AMINO_ACIDS)
    
    # Extract ligand and small molecule information
    ligand_atoms = [atom for atom in structure.get_atoms() if atom.parent.id[0] != ' ']

    if ligand_atoms:
        ligand_coords = torch.tensor([atom.coord for atom in ligand_atoms], dtype=torch.float)
    ...
    return Data(x=x, edge_index=data.edge_index, edge_attr=edge_attr, y=y,
                ligand_x=ligand_features, ligand_pos=ligand_coords, ligand_batch=ligand_batch)
...
def train(model, train_loader, optimizer, device):
    ...
    for data in train_loader:
        ...
        out = model(data.x, data.edge_index, data.edge_attr, data.batch,
                    data.ligand_x, data.ligand_batch)
...
def generate_sequence(model, data, device, temperature=1.0, top_k=None):
    ...
    with torch.no_grad():
        logits = model(data.x, data.edge_index, data.edge_attr, 
                       data.batch, data.ligand_x, data.ligand_batch)
        ...
        sequence_letters = list(IUPACData.protein_letters_3to1[resname.capitalize()] for resname in sequence_letters)
...
model = ProteinLigandGNN(protein_features=NUM_DIHEDRAL_FEATURES,
...
generated_sequence = generate_sequence(model, test_data, device, temperature=1.0, top_k=None)
